...
def process_image(input_path, output_path, low_thresh=50, high_thresh=150):
    ...
    # Build and execute the C++ command line execution
    cmd = f"{CPP_BINARY} {width} {height} {temp_in} {temp_out} {low_thresh} {high_thresh}"
    print(f"[*] Executing C++ Canny Pipeline: {cmd}")
    
    if os.system(cmd) != 0:
        print("Error: C++ application execution failed!")
        if os.path.exists(temp_in): os.remove(temp_in)
        sys.exit(1)

    # Read the generated raw edges back from disk
    if not os.path.exists(temp_out):
        print(f"Error: C++ app did not generate {temp_out}")
        if os.path.exists(temp_in): os.remove(temp_in)
        sys.exit(1)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 run_canny.py <input_image.jpg/png> <output_image.png> [low_thresh] [high_thresh]")
        sys.exit(1)
